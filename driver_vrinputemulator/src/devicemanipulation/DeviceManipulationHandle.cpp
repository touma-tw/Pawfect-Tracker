...
bool DeviceManipulationHandle::handlePoseUpdate(uint32_t& unWhichDevice, vr::DriverPose_t& newPose, uint32_t unPoseStructSize) {
	...
	// Cache the latest valid pose (used by other device modes below).
	...
	if (m_deviceMode == 1) { // fake disconnect mode
		...
		if (m_offsetsEnabled) {
			...
			// HMD-relative offset. m_deviceTranslationOffset holds a HEAD-LOCAL delta
			// (meters) precomputed by the client from FINAL runtime poses -- i.e. with
			// the correct HMD yaw, already through Space Calibrator and the recenter
			// transform that only the client can observe. Here we rotate it into this
			// device's driver-point frame using the live qDriverFromHeadRotation (which
			// only the driver can observe) and add it to vecDriverFromHeadTranslation.
			// Because OpenVR reconstructs world pos as R(qWorldFromDriver*qRotation) *
			// vecDriverFromHead, and R_head = R(qWorldFromDriver*qRotation)*R(qDriverFromHead),
			// this yields exactly the requested world delta regardless of hook order.
			if (m_deviceTranslationOffset.v[0] != 0.0 || m_deviceTranslationOffset.v[1] != 0.0 || m_deviceTranslationOffset.v[2] != 0.0) {
				const auto& q = newPose.qDriverFromHeadRotation;
				const auto& v = m_deviceTranslationOffset;
				double tx = 2.0 * (q.y * v.v[2] - q.z * v.v[1]);
				double ty = 2.0 * (q.z * v.v[0] - q.x * v.v[2]);
				double tz = 2.0 * (q.x * v.v[1] - q.y * v.v[0]);
				newPose.vecDriverFromHeadTranslation[0] += v.v[0] + q.w * tx + (q.y * tz - q.z * ty);
				newPose.vecDriverFromHeadTranslation[1] += v.v[1] + q.w * ty + (q.z * tx - q.x * tz);
				newPose.vecDriverFromHeadTranslation[2] += v.v[2] + q.w * tz + (q.x * ty - q.y * tx);
			}
